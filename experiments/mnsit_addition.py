...
def train_concept_bottleneck_model(train_dataset, test_dataset, extracting_model='resnet18', embedding_size=32,
    ...
    x_train, c_train, y_train = x_train.to(
        device), c_train.to(device), y_train.to(device)
    x_test, c_test, y_test = x_test.to(
        device), c_test.to(device), y_test.to(device)
    ...
    concept_embedder = te.nn.ConceptEmbedding(
        encoder_size, c_train.shape[1], embedding_size)
    ...
    for epoch in pbar:
        ...
        if bias:
            y_pred, weight_attn, bias_attn = task_predictor(
                c_emb, c_pred, return_attn=True)
        else:
            y_pred, weight_attn = task_predictor(
                c_emb, c_pred, return_attn=True)
        ...
        if bias:
            loss = concept_loss + weight_task * task_loss + \
                weight_reg * weight_attn_reg + bias_reg * \
                torch.mean(bias_attn.abs() ** 2)
        else:
        ...
        if epoch % 1 == 0:
            with torch.no_grad():
                ...
                task_acc = f1_score(
                    y_test.cpu(), y_pred.cpu().argmax(dim=1), average='macro')
                concept_acc = f1_score(
                    c_test.cpu(), c_pred.cpu() > 0.5, average='macro')
    ...
    global_explanations = task_predictor.explain(
        c_emb, c_pred, 'global', concept_names)
