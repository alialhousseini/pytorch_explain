...
class ConceptReasoningLayerMod(torch.nn.Module):
    def __init__(self, emb_size, n_classes, logic: Logic = GodelTNorm(), temperature: float = 100., log=False):
        ...
        self.selector_nn = torch.nn.Sequential(
            torch.nn.Linear(emb_size, emb_size),
            torch.nn.LeakyReLU(),
    ...
    def forward(self, x, c, return_attn=False, sign_attn=None, filter_attn=None):
        ...
        # Get selector outputs
        selector_outputs = torch.sigmoid(self.selector_nn(x))
        print(selector_outputs)
        print(selector_outputs.shape)

        if self.log:
            logger.info(f"Selector Outputs: {selector_outputs.shape}")

        preds = selector_outputs[:, 0] * c + selector_outputs[:, 1] * (1 - c)
    ...
    def explain(self, x, c, mode, concept_names=None, class_names=None, filter_attn=None):
        ...
        all_class_explanations = {cn: [] for cn in class_names}
        for sample_idx in range(len(x)):
            relevance = y_preds[sample_idx, 0] > 0.5
            sign = y_preds[sample_idx, 1] > 0.5
