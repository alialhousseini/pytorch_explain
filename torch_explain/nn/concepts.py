...
class ConceptLinearLayer(torch.nn.Module):
    ...
    def __init__(self, emb_size, n_classes, bias=True, attention=False, log=False):
        super().__init__()
        self.emb_size = emb_size
        self.n_classes = n_classes
        ...
        if attention:
            self.attention_nn = torch.nn.MultiheadAttention(
                emb_size, 1, batch_first=True)
        self.log = log
    ...
    def forward(self, x, c, return_attn=False, weight_attn=None):
        ...
        if self.attention:
            ...
            if self.log:
                logger.info(f"X after attention: {x.shape}")
        if weight_attn is None:

            weight_attn = self.pos_weight_nn(x) - self.neg_weight_nn(x)
            if self.log:
                logger.info(f"Weight Attn: {weight_attn.shape}")
                logger.info(f"Weight Attn: {weight_attn}")
        ...
        if self.log:
            ...
            logger.info(f"Logits: {logits}")
        if self.bias:
            bias_attn = self.pos_bias_nn(
                x.mean(dim=1)) - self.neg_bias_nn(x.mean(dim=1))
    ...
    def explain(self, x, c, mode, concept_names=None, class_names=None, weight_attn=None):
        ...
        # make a forward pass to get predictions and attention weights
        if self.bias:
            y_preds, weight_attn_mask, _ = self.forward(
                x, c, return_attn=True, weight_attn=weight_attn)
        else:
            y_preds, weight_attn_mask = self.forward(
                x, c, return_attn=True, weight_attn=weight_attn)
        ...
        for sample_idx in range(len(x)):
            ...
            if len(active_classes) == 0:
                ...
                for target_class in active_classes:
                    ...
                    for concept_idx in range(len(concept_names)):
                        c_pred = c[sample_idx, concept_idx]
                        weight_attn = weight_attn_mask[sample_idx,
                                                       concept_idx, target_class]
                        ...
                        if torch.abs(c_pred) > 0.5:
                            if weight_attn > 0:
                                minterm.append(
                                    f'+{weight_attn:.0f} {concept_names[concept_idx]}')
                            else:
                                minterm.append(
                                    f'{weight_attn:.0f} {concept_names[concept_idx]}')
